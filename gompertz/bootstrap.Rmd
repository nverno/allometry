...
```{r params}
renderTable({ as.data.frame(params$seed) }, digits=4)
```
...
shinyUI(fluidPage(
    ...
    fluidRow(
        column(6, sliderInput("R", "Number of bootstraps", min=1, max=1000, value=1)),
        column(6, sliderInput("n", "Number samples", min=1, max=2000, value=nrow(pp)))
    ),
    fluidRow(
...
shinyUI(fluidPage(
    ...
    fluidRow(
        column(6, sliderInput("numSizes", "Number of size classes:", min=0, max=100, value=20))
...
senseResults <- reactive({
    if (input$run_sense) {
        ...
        res$bins <- as.factor(rep(rep(1:bins, each=reps+1), times=length(ps)))
        res
    } else "Not Run"
})
...
traceCoefs <- renderPlot({
    ...
    res <- isolate(senseResults())
    if (is.list(res)) {
        ests <- res %>% group_by(bins, coef) %>% summarise(estimate=mean(value))
        pars <- input$tracePars
        p <- ggplot(res[res$coef %in% pars,], aes(bins, value, group=coef, color=coef)) +
...
shinyUI(fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            checkboxGroupInput("tracePars", "Trace parameters:", names(ps), selected=c("a"),
                               inline=T),
